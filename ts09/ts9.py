...
# AGREGADO HERNAN
lp_sos_butter = sig.iirdesign(wp = wp1/nyq_frec, ws = ws1/nyq_frec, gpass=0.5, gstop=40., analog=False, ftype='butter', output='sos')
hp_sos_butter = sig.iirdesign(wp = wp2/nyq_frec, ws = ws2/nyq_frec, gpass=0.5, gstop=40., analog=False, ftype='butter', output='sos')
sos_bp = np.vstack([lp_sos_butter, hp_sos_butter])
# FIN AGREGADO HERNAN
...
# AGREGADO HERNAN
_, h_butter_lp = sig.sosfreqz(lp_sos_butter, w_rad)
_, h_butter_hp = sig.sosfreqz(hp_sos_butter, w_rad)
_, h_butter_bp = sig.sosfreqz(sos_bp, w_rad)
# FIN AGREGADO HERNAN
...
# YA ESTABA COMENTADO # plt.plot(w, 20 * np.log10(abs(hh_remez_hp)), label='FIR-remez-HP {:d}'.format(num_remez_hp.shape[0]))
# YA ESTABA COMENTADO # plt.plot(w, 20 * np.log10(abs(hh_remez_lp)), label='FIR-remez-LP {:d}'.format(num_remez_lp.shape[0]))
...
# AGREGADO HERNAN
plt.plot(w, 20*np.log10(np.abs(h_butter_lp)+1e-12), label='IIR-Butter LP {:d}'.format(lp_sos_butter.shape[0]*2) )
plt.plot(w, 20*np.log10(np.abs(h_butter_hp)+1e-12), label='IIR-Butter HP {:d}'.format(hp_sos_butter.shape[0]*2) )
plt.plot(w, 20*np.log10(np.abs(h_butter_bp)+1e-12), label='IIR-Butter BP {:d}'.format(sos_bp.shape[0]*2) )
# FIN AGREGADO HERNAN
...
# AGREGADO HERNAN
ECG_f_butt_LP = sig.sosfiltfilt(lp_sos_butter, ecg_one_lead)
ECG_f_butt_HP = sig.sosfiltfilt(hp_sos_butter, ecg_one_lead)
ECG_f_butt_BP = sig.sosfiltfilt(sos_bp, ecg_one_lead)
# FIN AGREGADO HERNAN
...
for ii in regs_interes:
    ...
    # AGREGADO HERNAN
    plt.plot(zoom_region, ECG_f_butt_BP[zoom_region], label='Butter BP')
    # FIN AGREGADO HERNAN
    
    # FIR con corrección de demora
    # YA ESTABA COMENTADO # plt.plot(zoom_region, ECG_f_remez[zoom_region+demora], label='Remez')
    # YA ESTABA COMENTADO # plt.plot(zoom_region, ECG_f_ls[zoom_region+demora], label='LS')
    # YA ESTABA COMENTADO # plt.plot(zoom_region, ECG_f_win[zoom_region+demora], label='Win')
